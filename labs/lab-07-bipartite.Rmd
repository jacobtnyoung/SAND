...
bipartite.example.net <- as.network(
  ...
  # here, we define the number of nodes that are in the first mode
  bipartite = dim( bipartite.example )[1]
  )

# there 11 vertices, 6 are bipartite (in the first mode), and 12 edges
bipartite.example.net

# Another way of build this
bipartite.example2.net <- as.network(
  bipartite.example,
  # add the first and second dimension
  bipartite=( dim( bipartite.example )[1] + dim( bipartite.example )[2] )
  )

# there 11 vertices, 6 are bipartite (in the first mode), and 12 edges
bipartite.example2.net
...
url <- ( "https://raw.githubusercontent.com/jacobtnyoung/SAND/main/data/lab_data/lab-07-officer-event-edgelist.csv" )

officer.event.edgelist   <- as.matrix(read.csv(url, as.is=TRUE, header=TRUE))
