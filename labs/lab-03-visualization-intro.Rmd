...
**FOR THIS LAB, YOU NEED TO GO TO THE LAYERING PIECE FROM THE R WORKSHOP SO YOU CAN GET THAT CODE AND WORK THE LOGIC IN HERE**
...
Next, let's get the `UserNetR` package from Douglas Luke's GitHub page. Since this package is not on the CRAN package repository, we need to install it from Github. We do this in four steps:  
...
To see the various functionality of the function, let's work with the example of an undirected network from [lab 02](). 


```{r,echo=TRUE,eval=TRUE,message=FALSE}

# define the path
url <- "https://raw.githubusercontent.com/jacobtnyoung/SAND/main/data/lab_data/lab-02-undirected-example.csv"
...
net.u <- as.network( 
  as.matrix(
    read.csv( 
      "https://raw.githubusercontent.com/jacobtnyoung/SAND/main/data/lab_data/lab-02-undirected-example.csv",
      as.is = TRUE,
      header = TRUE,
      row.names = 1 ) ),
  directed = FALSE 
...
Alternatively, we could add in a string of names for the label, `gplot( net.u, gmode = "graph",  label = c( "Jen", "Tom", "Bob", "Leaf", "Jim" ) )`.  
Or we could read them in as an object: `names <- c( "Jen", "Tom", "Bob", "Leaf", "Jim" )` and then to the plot using `gplot( net.u, gmode = "graph", label = names )`.  
...
A great feature of R is that we can tune the graphing parameters. Here are several examples: 
  ...
  + Labels:
    ...
    + Change label size, `label.cex = 1.5`
  
    + Color the labels, `label.col = "blue"`


HERE WITH REFORMATTING


* What if we wanted different colored names? Just enter the colors as a variable using the `c()` function, `gplot( net.u, gmode = "graph", boxed.labels = TRUE, label.cex = 1.5, label.col = c( "red", "blue", "green", "orange", "grey" ), label = network.vertex.names( net.u ) )`.
